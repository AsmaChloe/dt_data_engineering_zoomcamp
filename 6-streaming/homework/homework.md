...
```bash
redpanda@c0f18ffff7d9:/$ rpk version
v22.3.5 (rev 28b2443)
```
...
```bash
redpanda@c0f18ffff7d9:/$ rpk topic create test-topic
TOPIC       STATUS
test-topic  OK`
```
...
Sent: {'number': 8}
Sent: {'number': 9}
Sending messages took 0.51327 seconds
Flusing took 0.00010 seconds
Total process took 0.51380 seconds
```
...
It took a total of 0.51 seconds 
...
See file [send_taxi_data.py](./send_taxi_data.py).
...
Creation of topic : 
```bash 
redpanda@c0f18ffff7d9:/$ rpk topic create green-trips
TOPIC        STATUS
green-trips  OK
```
Sending data : 

See sending script : [send_taxi_data.py](./send_taxi_data.py)
```bash
$ python send_taxi_data.py 
Sending...
Sending messages took 40.41209 seconds
```
It took 40 seconds.
...
The data is not in byte format now.

Before : 
```python
Row(key=None, value=bytearray(b'{"lpep_pickup_datetime": "2019-10-01 00:26:02", "lpep_dropoff_datetime": "2019-10-01 00:39:58", "PULocationID": 112, "DOLocationID": 196, "passenger_count": 1.0, "trip_distance": 5.88, "tip_amount": 0.0}'), topic='green-trips', partition=0, offset=0, timestamp=datetime.datetime(2024, 4, 7, 11, 43, 38, 806000), timestampType=0)
```
...
See [consumer.ipyng](./consumer.ipynb).

Zone 74 est the most popular destionation ID
